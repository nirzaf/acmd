...
CREATE TABLE IF NOT EXISTS tbl_users (user_id INTEGER PRIMARY KEY ASC AUTOINCREMENT UNIQUE NOT NULL, username VARCHAR (30) NOT NULL UNIQUE, password VARCHAR (30) NOT NULL, user_type INTEGER NOT NULL ON CONFLICT ROLLBACK, status BOOLEAN DEFAULT (0) NOT NULL ON CONFLICT ROLLBACK);
...
CREATE TABLE IF NOT EXISTS tbl_property_types (type_id INTEGER PRIMARY KEY ASC AUTOINCREMENT NOT NULL ON CONFLICT ROLLBACK, type_name int (11) NOT NULL, isDeleted BOOLEAN DEFAULT (1));
